    ...
    const IntegrationCard: React.FC<{ schema: IntegrationSchema }> = ({ schema }) => {
        ...
        return (
            <div className={`p-4 rounded-xl border transition-all ${schema.isConnected ? 'bg-slate-900/50 border-green-900/50' : 'bg-slate-950 border-slate-800'}`}>
                ...
                {isEditing && (
                    <div className="mt-4 space-y-3 bg-black/20 p-3 rounded border border-slate-800 animate-in fade-in slide-in-from-top-2">
                        {schema.fields.map(field => {
                            ...
                            return (
                                <div key={field.key}>
                                    <label className="text-[10px] uppercase text-slate-500 font-bold block mb-1 flex justify-between">
                                        <span>{field.label} {field.required && '*'}</span>
                                        {hasValue && <span className="text-green-500 flex items-center gap-1"><Check size={8}/> SAVED</span>}
                                    </label>
                                    <div className="relative">
                                        <input 
                                            ...
                                            className={`w-full bg-slate-900 border rounded p-2 text-xs text-white focus:outline-none font-mono placeholder:text-slate-600 ${hasValue ? 'border-green-900/30' : 'border-slate-700 focus:border-cyan-500'}`}
                                        />
                                        {field.type === 'password' && (
                                            <button 
                        ...
                        <div className="flex justify-end gap-2 mt-2">
                            {schema.isConnected && (
                                <button 
                                    ...
                                    className="px-3 py-1.5 bg-red-900/20 text-red-400 hover:bg-red-900/40 rounded text-xs border border-red-900/50"
                                >
                                    DISCONNECT
                                </button>
                            )}
                            <button 
                                ...
                                className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded text-xs font-bold"
                            >
                                SAVE & CONNECT
                            </button>
                        </div>
                                    ...
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className={`flex-1 py-2 rounded text-xs font-bold border transition-all ${
                                                settings.theme.density === 'comfortable' 
